...
export default function ProductCard({ item }: ProductCardProps) {
  const prodId = item.id.split("/").pop();
  return (
    <li
      key={item.id}
      className="relative border border-gray-200 rounded-lg overflow-hidden shadow-sm hover:shadow-lg transition-shadow duration-300 ease-in-out group"
    >
      ...
      <div className="p-4">
        ...
        <h4 className="text-gray-500 mt-1">
          {formatPrice(item.priceRangeV2.minVariantPrice.amount, item.priceRangeV2.minVariantPrice.currencyCode)}
